...
<form method="post" action="<%= request.getContextPath() %>/event/event_search_list.jsp">
...
<% 
    ...
    String getSearchKey = request.getParameter("searchKey");
    String getSearchValue = request.getParameter("searchValue"); 

    ArrayList<String[]> getEventSearch = new ArrayList<String[]>();
    
    Edao edao = new Edao();

    getEventSearch = edao.mEventSearch(getSearchKey, getSearchValue);
        
    if(getEventSearch.size() == 0){
%>
       <tr>
           <td></td>
           <td></td>
           <td style="text-align:center;">검색 결과 없음</td>
           <td></td>
           <td></td>
        </tr>
<%
